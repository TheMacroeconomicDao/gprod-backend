...
const USERS_SEARCH = [
  { username: 'alice_search', email: 'alice_search@test.com', password: '123456' },
  { username: 'bob_search', email: 'bob_search@test.com', password: '123456' },
  { username: 'vasya_search', email: 'vasya_search@test.com', password: '123456' },
];
...
describe('Users search/sort (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const foundUser = usersArr.find((u: any) => u.username === lastUser.username);
    if (!foundUser) {
      throw new Error(`Пользователь ${lastUser.username} не найден в списке пользователей`);
    }
    
    userId = foundUser.id;
    console.log(`Используем пользователя ${lastUser.username} с ID: ${userId} для тестов поиска`);
  });
  ...
  it('поиск по username', async () => {
    ...
    const foundUser = res.body.data.find((u: any) => u.username === searchUsername);
  ...
  it('сортировка по username:desc', async () => {
    ...
    const vasyaIndex = usernames.findIndex((name: string) => name === USERS_SEARCH[2].username);
    const bobIndex = usernames.findIndex((name: string) => name === USERS_SEARCH[1].username);
    const aliceIndex = usernames.findIndex((name: string) => name === USERS_SEARCH[0].username);
    ...
    if (vasyaIndex !== -1 && bobIndex !== -1 && aliceIndex !== -1) {
      ...
      console.warn('Не все тестовые пользователи найдены в результатах сортировки:', usernames);
    }
  });
