...
### Задача: Исправить автоматизацию тестирования и запуск контейнеров

**Описание:**  
В текущей автоматизации тесты не запускаются, если контейнер `app` не поднят или падает сразу после старта.  
Скрипты `run.sh` и `run-new.sh` ведут себя по-разному:  
- `run-new.sh` требует, чтобы контейнеры были уже подняты (иначе тесты не идут).
- `run.sh` требует шаблон `.env.test`, которого может не быть.

**Чеклист:**
- [ ] Унифицировать поведение скриптов: тесты должны запускаться только после успешного старта контейнеров.
- [ ] Добавить явную проверку статуса контейнера `app` перед запуском тестов.
- [ ] В случае падения контейнера — выводить логи и причину ошибки.
- [ ] Для `run.sh` — автоматизировать генерацию `.env.test` или отказаться от необходимости шаблона.
- [ ] Для `run-new.sh` — добавить автоматический запуск контейнеров, если они не подняты.
- [ ] Обновить документацию и onboarding: чётко описать порядок действий для тестирования.
- [ ] Добавить в CI/CD шаги проверки статуса контейнеров и автоматического старта перед тестами.
