...
- Клонируй репозиторий и установи зависимости через pnpm.
- Для настройки окружения всегда используй скрипты из `automation/env/`:
  ```sh
  ./automation/env/env-manager.sh dev --docker
  # или интерактивно
  ./automation/env/env-switch-new.sh
  ```
- Не редактируй .env вручную — он всегда генерируется или переключается автоматически.
- Для запуска тестов и разработки всегда используй актуальный .env через эти скрипты.

## CI/CD: env и тесты

- В CI/CD пайплайне обязательно добавляй шаг генерации .env через скрипты из `automation/env/`.
- Пример для GitHub Actions:
  ```yaml
  - name: Setup env
    run: ./automation/env/env-manager.sh dev --docker
  ```
- Для запуска тестов в Docker:
  ...
  ```
- Убедись, что база и app описаны в docker-compose, все dev-зависимости установлены.
