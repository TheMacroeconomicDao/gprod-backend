...
export class UploadService {
  ...
  private async ensureUploadDirExists(): Promise<void> {
    try {
      if (!await existsAsync(this.uploadDir)) {
        ...
        this.logger.log(`Создана директория для загрузки файлов: ${this.uploadDir}`);
      }
    } catch (error) {
      this.logger.error(`Ошибка при создании директории для загрузки: ${error.message}`);
    }
  }
  ...
  async saveFile(
    ...
    try {
      ...
      this.logger.error(`Ошибка при сохранении файла: ${error.message}`, error.stack);
